...
export class TapMessagingService {
	...
	private async drainQueuedJournalCommands(): Promise<number> {
		...
		for (const entry of queued) {
			...
			const command = { type: commandType, payload: commandPayload } as TapCommandJobIntent;

			try {
				const result = await this.executeJournalCommand(command);
		...
		switch (command.type) {
			...
			case "send-message": {
				return await this.sendMessageInternal(
					command.payload.peer,
					command.payload.text,
					command.payload.scope,
					command.payload.autoGenerated ? { autoGenerated: true } : undefined,
				);
			}
			case "publish-grant-set": {
				return await this.publishGrantSetInternal(
					command.payload.peer,
					command.payload.grantSet,
			...
			case "request-funds": {
				return await this.requestFundsInternal(command.payload.input);
			}
			case "request-meeting": {
				return await this.requestMeetingInternal(command.payload.input);
			}
			default:
				throw new Error(`Unknown command type: ${String(command satisfies never)}`);
		}
	}
